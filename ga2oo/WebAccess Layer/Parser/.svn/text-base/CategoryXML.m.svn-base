...
@interface CategoryXML(LibXMLParserMethods)
...
-(void)endDocument
{
	if(self.delegate!=nil && [self.delegate respondsToSelector:@selector(CategoryXML_Finished:)])
	{
		[(id)[self delegate] performSelectorOnMainThread:@selector(CategoryXML_Finished:)withObject:arrData waitUntilDone:YES];
		[self autorelease];
	}
}
...
{
	[(id)[self delegate] CategoryXML_Error:self encounteredError:err];
}
