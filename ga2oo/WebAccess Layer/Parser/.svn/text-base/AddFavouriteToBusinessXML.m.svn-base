...
@interface AddFavouriteToBusinessXML(LibXMLParserMethods)
...
@implementation AddFavouriteToBusinessXML
...
- (BOOL)parseWithLibXML2Parser{
	...
	currentObject = [[AddFavouriteToBusinessBO alloc] init];
	appDelegate = (Ga2ooAppDelegate *)[[UIApplication sharedApplication]delegate];
	NSString *newstr = [[NSString alloc] initWithFormat:@"http://5.ga2ootesting.appspot.com/rest/v1/user/businesses/"];
...
-(void)endDocument
{
	[(id)[self delegate] performSelectorOnMainThread:@selector(AddFavouriteToBusinessXML_Finished:)withObject:currentObject waitUntilDone:YES];
	[self autorelease];
}

-(void)SaxXMLParserError:(SaxXMLParser *)parser EncounteredError:(NSError *)err
{
	[(id)[self delegate] AddFavouriteToBusinessXML_Error:self encounteredError:err with:currentObject];
}
