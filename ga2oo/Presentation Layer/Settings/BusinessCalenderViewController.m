//
//  BusinessCalenderViewController.m
//  Ga2oo
//
...
@implementation BusinessCalenderViewController
...
@synthesize TotalBusinessEvents;
...
-(void)SearchEventsBtnClicked
{
    ...
    if (CalenderSearchEnabled) {
		...
		NSArray *arr = [TotalBusinessEvents filteredArrayUsingPredicate:pred];
...
- (void)viewDidLoad {
    ...
    [arrCalernderEvents addObjectsFromArray:TotalBusinessEvents];
	countForArrow = 0;
...
-(void)checkForEvents
{
	...
	NSArray *arr_Eve = [TotalBusinessEvents filteredArrayUsingPredicate:pred_Date];
...
-(void)ListBtn_Clicked:(UIButton *)btn
{
    ...
    if (![selectedView isEqualToString:@"List"]) {
        ...
        NSArray *arr = [TotalBusinessEvents filteredArrayUsingPredicate:pred_Date];
        [arrListingEvents  addObjectsFromArray:arr];
...
-(IBAction) eventButton_onTouchUpInside:(id)sender{
	...
	EventDetailViewController *eventVC = [[EventDetailViewController alloc]initWithNibName:@"EventDetailViewController" bundle:[NSBundle mainBundle]];
	eventVC.objEvent = [TotalBusinessEvents objectAtIndex:btn.tag];
	[self.navigationController pushViewController:eventVC animated:YES];
	[eventVC release];
}
...
-(IBAction)btn_scrlActionTop:(UIButton *)sender
{
    ...
    if ([categoryId isEqualToString:@"All"]) {
        for (int i = 0; i<[arrCatEvents count]; i++) {
            ...
            NSArray *arr1 = [TotalBusinessEvents filteredArrayUsingPredicate:pred1];
            if ([arr1 count]) {
	...
	if(tableView.tag!=1111 && tableView.tag!=2222)
	{
		...
		if(tableView.tag == 1)
		{
			...
			else
			{
				...
				obj_Event=[TotalBusinessEvents objectAtIndex:indexPath.row];
	...
	if(tableView.tag!=1111 && tableView.tag!=2222)
	{
		...
		if(tableView.tag == 1)
		{
			...
			if(searching)
			{
				...
				EventDetailViewController *eventVC = [[EventDetailViewController alloc]initWithNibName:@"EventDetailViewController" bundle:[NSBundle mainBundle]];
 				eventVC.objEventList = [TotalBusinessEvents objectAtIndex:indexPath.row];
 				[self.navigationController pushViewController:eventVC animated:YES];
				[eventVC release];
			}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	if(alertView.tag==1234)
	{
        ...
        {
            objEvent=[TotalBusinessEvents objectAtIndex:intTagValue-100];
        }
