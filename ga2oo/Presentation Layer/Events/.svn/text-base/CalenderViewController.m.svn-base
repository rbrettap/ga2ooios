...
-(void)SearchEventsBtnClicked
{
	...
	//searchEventLocation = searchLocationList.text;
...
- (void)viewDidLoad {
    ...
    */
	[arrCalernderEvents addObjectsFromArray:appDelegate.arr_AllEvents];
	...
	{
		obj_Event=[appDelegate.arr_AllEvents objectAtIndex:i];
		dateForm = [[NSDateFormatter alloc] init];
		[dateForm setDateFormat:@"yyyy-MM-dd"];
		NSDate *eventdate = [dateForm dateFromString:obj_Event.event_start_date];
		[dateForm setDateFormat:@"dd/M/yyyy"];
		NSLog(@"%@", [dateForm stringFromDate:[NSDate date]]);
		if([[dateForm stringFromDate:[NSDate date]] isEqualToString:[dateForm stringFromDate:eventdate]])
		{
			[arr_Events addObject:obj_Event];
			if([obj_Event.is_featured isEqualToString:@"True"])
			{
				[arrListingEvents addObject:obj_Event];
			}
		}
		[dateForm release];
	}
    ...
    */
	...
	//NSSortDescriptor *nameSortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"maincategoryname" ascending:YES selector:@selector(localizedCaseInsensitiveCompare:)];
  	//[arr_Buttons sortUsingDescriptors:[NSArray arrayWithObjects:nameSortDescriptor,nil]];
...
-(void)checkForEvents
{
	...
	NSArray *arr_Eve = [appDelegate.arr_AllEvents filteredArrayUsingPredicate:pred_Date];
...
-(void)ListBtn_Clicked:(UIButton *)btn
{
    ...
    if (![selectedView isEqualToString:@"List"]) {
        ...
        NSArray *arr = [appDelegate.arr_AllEvents filteredArrayUsingPredicate:pred_Date];
        [arrListingEvents  addObjectsFromArray:arr];
...
-(void)CalBtn_Clicked:(UIButton *)btn
{
    ...
    if (![selectedView isEqualToString:@"Calender"])
    {
        ...
        [self performSelectorOnMainThread:@selector(LoadSelectedDateCalender) withObject:nil waitUntilDone:YES];
...
-(IBAction) eventButton_onTouchUpInside:(id)sender{
	...
	EventDetailViewController *eventVC = [[EventDetailViewController alloc]initWithNibName:@"EventDetailViewController" bundle:[NSBundle mainBundle]];
	eventVC.objEvent = [appDelegate.arr_AllEvents objectAtIndex:btn.tag];
	[self.navigationController pushViewController:eventVC animated:YES];
	[eventVC release];
}
...
-(IBAction)btn_scrlActionTop:(UIButton *)sender
{
    ...
    NSMutableArray *arrSelectedEvents = [[[NSMutableArray  alloc]init] autorelease];
		////  hilight selected button ////
	for(int i=0;i<[arr_ValTop count];i++)
	{
		[(UIButton *)[arr_ValTop objectAtIndex:i] setImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle]pathForResource:@"btn_hover1" ofType:@"png"]] forState:UIControlStateNormal];
    ...
    if (sender.tag !=0) {
        ...
        SelectedCategory = [categoryId integerValue];
    }
    else{
        categoryId = @"All";
        SelectedCategory = 0;
	...
	if(tableView.tag!=1111 && tableView.tag!=2222)
	{
		...
		if(tableView.tag == 1)
		{
			...
			else
			{
				...
				obj_Event=[appDelegate.arr_AllEvents objectAtIndex:indexPath.row];
				...
				if([obj_Event.UserID length])
				{
					...
					if(([arr count]-1)==1)
        					cell.lbl_fnds.text=[NSString stringWithFormat:@"%i friend", [arr count]-1];
		            else 
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath 
{
    if(tableView.tag!=1111 && tableView.tag!=2222)
    {
		...
		else if(tableView.tag == 2)
        {
			...
			{
                if([appDelegate.arr_AllEvents count])
                    [(EventsListCustomCell *)cell showEventsImage];
	...
	if(tableView.tag!=1111 && tableView.tag!=2222)
	{
		...
		if(tableView.tag == 1)
		{
			...
			if(searching)
			{
				...
				EventDetailViewController *eventVC = [[EventDetailViewController alloc]initWithNibName:@"EventDetailViewController" bundle:[NSBundle mainBundle]];
 				eventVC.objEventList = [appDelegate.arr_AllEvents objectAtIndex:indexPath.row];
 				[self.navigationController pushViewController:eventVC animated:YES];
				[eventVC release];
			}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(alertView.tag==1234)
	{
        ...
        {
            objEvent=[appDelegate.arr_AllEvents objectAtIndex:intTagValue-100];
        }
...
-(void)laodEventBySearch
{
  
    NSString *Temp = [selectedDate stringByReplacingOccurrencesOfString:@"-" withString:@""];
    NSLog(@"Temp %@",Temp);
    NSLog(@"selectedDate %@",selectedDate);
    EventsLayer.ModeType = GetBycategory_dateBysortOption;
   /* if([sortOption length]>2)
    {
      
        
        NSRange rangeIs = [sortOption rangeOfString:@"Others" options:NSCaseInsensitiveSearch]; 
        if(rangeIs.length  > 3 )
        {
        ...
        {
          EventsLayer.str_type = sortOption;
        }
        
        NSRange rangeIs2 = [sortOption rangeOfString:@"distance" options:NSCaseInsensitiveSearch]; 
        if((rangeIs.length  > 3 ) && (![EventsLayer.str_type length]))
        {
            EventsLayer.str_type = [NSString stringWithFormat:@""];
        }
        else 
    ...
    EventsLayer.str_type = [NSString stringWithFormat:@""];
    EventsLayer.str_date = Temp;
    if(SelectedCategory)
    {
     EventsLayer.str_category = [NSString stringWithFormat:@"%i",SelectedCategory];
    }
    else
    {
...
{
    [arrListingEvents removeAllObjects];
    [arrListingEvents addObjectsFromArray:arrdata];
    if(![arrListingEvents count])
    {
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:nil message:@"No events found." delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alert show];
