...
@implementation CalenderTabViewController
...
static int shouldLoadCalender= 1;
...
-(void)SearchEventsBtnClicked
{
    ...
    if (CalenderSearchEnabled) 
	{
		...
		NSArray *arr = [appDelegate.arrFavorite filteredArrayUsingPredicate:pred];
	...
	if ([selectedView isEqualToString:@"Calender"])
    {
        ...
        for(int i=0;i<[arrSearchedEvents count];i++)
        {
            ...
            NSDate *eventdate = [dateFormatter dateFromString:obj_.event_start_date];
            [dateFormatter setDateFormat:@"dd/M/yyyy"];
            NSLog(@"%@", [dateFormatter stringFromDate:[NSDate date]]);
            if([[dateFormatter stringFromDate:[NSDate date]] isEqualToString:[dateFormatter stringFromDate:eventdate]])
            {
...
- (void)viewDidLoad {
    ...
    [super viewDidLoad];
	shouldLoadCalender = 1;
    
	[self setTopBarImageToView:self.view];
    friendbl = [[FriendsBL alloc]init];
	...
	NSString *dateString11 = [dateFormatter stringFromDate:today1];
	lbl_Date.text = [self getDate:dateString11];
 	
 	[dateFormatter setDateFormat:@"MMM"];
	lbl_Month.text = [dateFormatter stringFromDate:[NSDate date]];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if(shouldLoadCalender)
    {
        ...
        [arrCalernderEvents addObjectsFromArray:appDelegate.arrFavorite];
...
{	
	...
	NSDate *date=[dateFormatter dateFromString:[NSString stringWithFormat:@"%i %i %i",currentYear,currentMonth,currentDayRunning]];
	[dateFormatter setDateFormat:@"e"];
	currentWeekDay = [[dateFormatter stringForObjectValue:date]intValue];
...
-(void)checkForEvents
{
	...
	NSArray *arr_Eve = [appDelegate.arrFavorite filteredArrayUsingPredicate:pred_Date];
...
-(void) pushViewEventDetail:(NSString *)selectedDay
{
	...
	if(![selectedDay integerValue]==0)
	{
		...
		[dateFormatter setDateFormat:@"dd/M/yyyy"];
		NSDate *date = [dateFormatter dateFromString:selectedDay];
		
		[dateFormatter setDateFormat:@"yyyy-MM-dd"];
		NSString *strDate123 = [dateFormatter stringFromDate:date];
		...
		[self.arr_Events addObjectsFromArray:arr_Eve];
 		...
 		[dateFormatter setDateFormat:@"dd/M/yyyy"];
		datePicker.date= [dateFormatter dateFromString:selectedDay];
		NSLog(@"%@", selectedDay);
 
 		[dateFormatter setDateFormat:@"d"];
		NSString *dateString11 = [dateFormatter stringFromDate:datePicker.date];
		lbl_Date.text = [self getDate:dateString11];
 		
 		[dateFormatter setDateFormat:@"MMM"];
		lbl_Month.text = [dateFormatter stringFromDate:datePicker.date];
...
-(void)btn_FavDelete2:(NSInteger) sender
{
    intTagValue = 0;
	intTagValue = sender;
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"Do you want to remove this event form Palender?" delegate:self cancelButtonTitle:nil otherButtonTitles:@"Yes", @"No", nil];
	alert.tag = 1236;
	[alert show];
	[alert release];
}
    ...
    if (![selectedView isEqualToString:@"List"]) {
        ...
        if (![selectedDate length]) {
           ...
           // selectedDate = [dateFormat stringFromDate:today1];
            selectedDate = [[NSString alloc]initWithFormat:@"%@",[dateFormatter stringFromDate:today1]];
        ...
        NSArray *arr = [appDelegate.arrFavorite filteredArrayUsingPredicate:pred_Date];
        [arrListingEvents  addObjectsFromArray:arr];
		
       
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSDate *today1 = [dateFormatter dateFromString:selectedDate];
        [dateFormatter setDateFormat:@"d"];
        NSString *dateString11 = [dateFormatter stringFromDate:today1];
        lbl_Date.text = [self getDate:dateString11];
         
         [dateFormatter setDateFormat:@"MMM"];
        lbl_Month.text = [dateFormatter stringFromDate:today1];
...
-(IBAction) eventButton_onTouchUpInside:(id)sender{
	UIButton *btn = (UIButton *)sender;
     shouldLoadCalender = 0;
	EventDetailViewController *eventVC = [[EventDetailViewController alloc]initWithNibName:@"EventDetailViewController" bundle:[NSBundle mainBundle]];
	eventVC.objEvent = [appDelegate.arrFavorite objectAtIndex:btn.tag];
	[self.navigationController pushViewController:eventVC animated:YES];
	[eventVC release];
}
...
-(IBAction)btn_scrlActionTop:(UIButton *)sender
{
    ...
    if ([categoryId isEqualToString:@"All"]) {
        for (int i = 0; i<[arrCatEvents count]; i++) {
            ...
            NSArray *arr1 = [appDelegate.arrFavorite filteredArrayUsingPredicate:pred1];
            if ([arr1 count]) {
    ...
    if (tableView.tag!=1111 && tableView.tag!=2222) {
		...
		if(tableView.tag == 1)
		{
			...
			if([arr_Events count])
			{
				...
				[self btn_FavDelete2:(indexPath.row + 100)];
			}
		}
    }
}
	...
	if(tableView.tag!=1111 && tableView.tag!=2222)
	{
		...
		if(tableView.tag == 1)
		{
			if([arr_Events count])
			{
				...
				[cell setTips:obj];
                cell.lblTitle.frame = CGRectMake(55,1,200,44);
               ...
               /* UIButton *playbtn = [[UIButton alloc]initWithFrame:CGRectMake(250,10,41,26)];
                ...
                [playbtn addTarget:self action:@selector(btn_FavDelete2:) forControlEvents:UIControlEventTouchUpInside];
			...
			else
			{
				...
				obj_Event=[appDelegate.arrFavorite objectAtIndex:indexPath.row];
                ...
                {*/
                    ...
                    UIButton *playbtn = [[UIButton alloc]initWithFrame:CGRectMake(215,45,41,26)];
                    playbtn.tag=indexPath.row + 100;
	...
	if(tableView.tag!=1111 && tableView.tag!=2222)
	{
		...
		if(tableView.tag == 1)
		{
			...
			if(searching)
			{
				...
				shouldLoadCalender = 0;
				EventDetailViewController *eventVC = [[EventDetailViewController alloc]initWithNibName:@"EventDetailViewController" bundle:[NSBundle mainBundle]];
 				eventVC.objEventList = [appDelegate.arrFavorite objectAtIndex:indexPath.row];
 				[self.navigationController pushViewController:eventVC animated:YES];
				[eventVC release];
			}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	if(alertView.tag==1234)
	{
        ...
        {
            objEvent=[appDelegate.arrFavorite objectAtIndex:intTagValue-100];
        }
    ...
    else if(alertView.tag == 1235)
    {
     ...
     {
        objEvent=[arr_Events objectAtIndex:intTagValue-100];
...
-(void)httpResponseReceived:(WebResponse *)response
{
    ...
    if (response.tag == 10)
	{
		...
		[self FavoriteEventDeletCompleted2:strMsg];
    }
...
-(void)FavoriteEventDeletCompleted2:(NSString*)strMsgs
{
    NSRange rangeIs = [strMsgs rangeOfString:@"successfully" options:NSCaseInsensitiveSearch];
    if(rangeIs.length > 11)
    {
        for(int k = 0 ; [appDelegate.arr_FavoritesID count] > k ; k++)
        {
            ...
            if([tempid isEqualToString:objEvent.Eventid])
            {
                ...
                [arrCalernderEvents removeObject:objEvent];
                [view2 fillCalender:self.currentMonth weekStarts:self.currentFirstDay year:self.currentYear withEvents:arrCalernderEvents];
...
-(void)datePickerAction:(id)sender
{
	selected_Picker=3;
     ...
     [dateFormatter setDateFormat:@"dd/M/yyyy"];
	NSLog(@"selected Date:%@",[dateFormatter stringFromDate:datePicker.date]);
 }
...
-(void)doneClicked
{
	...
	{
         [dateFormatter setDateFormat:@"yyyy-MM-dd"];
		NSString *strDate123 = [dateFormatter stringFromDate:datePicker.date];
 		selectedDate = [[NSString alloc]initWithFormat:@"%@",strDate123];
        //[selectedDate release];
 		searching=YES;
...
-(void)LoadSelectedDateCalender
{
	...
	NSDate	*selectedToDate=[dateFormatter dateFromString:selectedDate];
	...
	NSInteger selectedcurrentDayRunning = [[dateFormatter stringForObjectValue:selectedToDate]intValue];
...
-(void)setCalendar
{
	...
	[gregorian release];
 	...
 	[dateFormatter setDateFormat:@"e"];
	currentWeekDay = [[dateFormatter stringForObjectValue:[NSDate date]]intValue];
 	
 	[dateFormatter setDateFormat:@"d"];
	currentDayRunning = [[dateFormatter stringForObjectValue:[NSDate date]]intValue];
